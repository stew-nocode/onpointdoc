...
/**
 * Route API pour lister les tâches avec pagination
 * 
 * Pattern similaire à /api/activities/list pour cohérence
 * 
 * Query params :
 * - offset: number (défaut: 0)
 * - limit: number (défaut: 25)
 * - search: string (recherche par titre et description)
 * - quick: TaskQuickFilter (filtre rapide)
 * - profileId: string (ID du profil utilisateur pour filtres conditionnels)
 * 
 * @param request - Requête Next.js
 * @returns Réponse JSON avec tâches paginées
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const quickFilter = searchParams.get('quick') as TaskQuickFilter | null;
    
    // Récupérer le tri depuis l'URL
    const sortParam = searchParams.get('sort');
    const sort = parseTaskSort(sortParam || undefined);
    ...
    const validQuickFilters: TaskQuickFilter[] = ['all', 'mine', 'todo', 'in_progress', 'blocked', 'completed', 'overdue'];
