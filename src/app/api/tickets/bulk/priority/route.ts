...
/**
 * Route API pour mettre à jour la priorité de plusieurs tickets en masse
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Validation avec Zod
    const validated = bulkUpdatePrioritySchema.parse(body);
    
    // Utiliser le service pour la logique métier
    const result = await bulkUpdatePriority(validated);
