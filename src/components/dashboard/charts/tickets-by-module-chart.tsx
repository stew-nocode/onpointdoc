...
type TicketsByModuleChartProps = {
  data: TicketsByModuleStats | null;
  className?: string;
};
...
/**
 * Chart - Tickets par module (Vertical Grouped Bar)
 * 
 * Affiche les modules avec 3 barres verticales groupées (BUG | REQ | ASSISTANCE).
 * 
 * @see docs/dashboard/REFONTE-DASHBOARD-SPECIFICATION.md - Section 3.3
 */
export function TicketsByModuleChart({ data, className }: TicketsByModuleChartProps) {
  // Mémoiser les données pour Recharts
  const chartData = useMemo(() => {
    if (!data?.data?.length) return [];
    
    // Données déjà triées par le service
    return data.data;
  }, [data?.data]);
  ...
  if (!data || data.data.length === 0) {
    return <TicketsByModuleChartEmpty className={className} />;
  }
      ...
      <CardContent className="pb-4">
        <ChartContainer config={chartConfig} className="w-full" style={{ height: chartHeight }}>
          ...
          >
            ...
            {/* Barres groupées (non empilées) - 3 barres par module */}
            <Bar
              dataKey="bug"
              fill={BAR_COLORS.bug}
              radius={[4, 4, 0, 0]}
 ...
 * État vide
 */
function TicketsByModuleChartEmpty({ className }: { className?: string }) {
  return (
    <Card className={cn(
      'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950',
      className
 ...
 */
export function TicketsByModuleChartSkeleton({ className }: { className?: string }) {
  return (
