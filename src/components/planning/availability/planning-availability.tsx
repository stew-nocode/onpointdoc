...
import { User, Clock, AlertCircle, CheckCircle2 } from 'lucide-react';
import { Badge } from '@/ui/badge';
import { Card, CardContent, CardHeader, CardTitle } from '@/ui/card';
import { Separator } from '@/ui/separator';
import { calculateAvailabilityForDate, getMockPeople } from './mock-data';
 ...
 * pour la date sélectionnée, basée sur la durée estimée (Option 3)
 */
export function PlanningAvailability({ selectedDate }: PlanningAvailabilityProps) {
  const people = getMockPeople();
  
  const availability = useMemo(() => {
    return calculateAvailabilityForDate(selectedDate, people);
  }, [selectedDate, people]);
  ...
  const getStatusBadge = (status: PersonAvailability['status']) => {
    switch (status) {
      case 'available':
        return (
  ...
  return (
    <Card className="h-full flex flex-col border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60">
