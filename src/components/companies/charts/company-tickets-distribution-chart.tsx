...
import type { CompanyTicketsDistributionStats } from '@/services/companies/stats/company-tickets-distribution-stats';
import {
  PIE_OUTER_RADIUS,
  PIE_INNER_RADIUS,
  ANIMATION_DURATION,
...
type CompanyTicketsDistributionChartProps = {
  data: CompanyTicketsDistributionStats | null;
  className?: string;
};
...
/**
 * Chart - Distribution des tickets par type pour une entreprise (PieChart Donut)
 *
 * Affiche la répartition BUG/REQ/ASSISTANCE sous forme de donut chart.
 * Pattern identique au dashboard pour cohérence UI/UX.
 *
 * @see src/components/dashboard/charts/tickets-distribution-chart.tsx
 */
export function CompanyTicketsDistributionChart({ 
  data, 
  className 
  ...
  if (!data || data.total === 0) {
    return <CompanyTicketsDistributionChartEmpty className={className} />;
  }
  ...
  return (
    <Card 
      ...
      style={{ 
        minWidth: `${COMPANY_CHART_MIN_WIDTH_DONUT}px`,
 ...
 * État vide du chart
 */
function CompanyTicketsDistributionChartEmpty({ className }: { className?: string }) {
  return (
    <Card 
      className={cn(
        'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950',
 ...
 */
export function CompanyTicketsDistributionChartSkeleton({ className }: { className?: string }) {
  return (
    <Card 
      className={cn(
        'animate-pulse flex-1', // Prend toute la largeur disponible si seul sur la ligne
        className
      )}
      style={{ 
        minWidth: `${COMPANY_CHART_MIN_WIDTH_DONUT}px`,
