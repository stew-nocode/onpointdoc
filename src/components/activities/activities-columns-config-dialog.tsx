...
import {
  ...
  saveVisibleActivityColumns,
  resetActivityColumnsToDefault,
  type ActivityColumnId
} from '@/lib/utils/activity-column-preferences';

type ActivitiesColumnsConfigDialogProps = {
  onColumnsChange?: (visibleColumns: Set<ActivityColumnId>) => void;
};

/**
 * Composant de dialogue pour configurer les colonnes visibles dans le tableau des activités
 * 
 * Pattern similaire à ColumnsConfigDialog pour cohérence
 * 
 * Principe Clean Code :
 ...
 */
export function ActivitiesColumnsConfigDialog({ 
  onColumnsChange 
}: ActivitiesColumnsConfigDialogProps) {
  ...
  const handleToggleColumn = (columnId: ActivityColumnId) => {
    const column = AVAILABLE_ACTIVITY_COLUMNS.find(col => col.id === columnId);
  ...
  const handleSave = () => {
    saveVisibleActivityColumns(visibleColumns);
    onColumnsChange?.(visibleColumns);
    setOpen(false);
  };

  const handleReset = () => {
    const defaultColumns = resetActivityColumnsToDefault();
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          ...
          size="sm"
          className="gap-2"
          onClick={() => setVisibleColumns(getVisibleActivityColumns())}
        >
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Choisissez les colonnes à afficher dans le tableau des activités.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
          <div className="max-h-[400px] overflow-y-auto space-y-2">
            {AVAILABLE_ACTIVITY_COLUMNS.map(column => {
