...
/**
 * Script d'import des contacts clients EDIPRESSE
 * 
 * Usage: node scripts/import-contacts-edipresse.js
 * 
...
// Données des contacts EDIPRESSE
const contactsData = [
  { "Nom Complet": "ZAGBAYOU ANNE", "Email": "anne.zagbayou@edipresse.ci", "Rôle": "Responsable Administrative" },
  { "Nom Complet": "YAO épouse KOSSONOU N", "Email": "", "Rôle": "" },
  { "Nom Complet": "Marius Coffi", "Email": "marius.coffi@edipresse-ci.ci", "Rôle": "Responsable Commercial" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise EDIPRESSE...\n`);

  // Récupérer l'ID de l'entreprise EDIPRESSE
  const { data: companies, error: companyErr } = await supabase
    .from('companies')
    .select('id, name, jira_company_id')
    .ilike('name', '%edipresse%');

  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise EDIPRESSE non trouvée`);
  ...
  const edipresseCompany = companies[0];
  console.log(`✅ Entreprise EDIPRESSE trouvée (ID: ${edipresseCompany.id}, Nom: ${edipresseCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', edipresseCompany.id)
          .single();
        existing = data;
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          full_name: fullName,
          role: 'client',
          company_id: edipresseCompany.id,
