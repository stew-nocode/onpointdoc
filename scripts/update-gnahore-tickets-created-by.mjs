...
/**
 * Script pour mettre à jour created_by des tickets créés par GNAHORE AMOS
 * 
 * Processus:
 ...
 * 3. Met à jour tickets.created_by avec le profil de GNAHORE AMOS
 * 
 * Usage:
 *   node scripts/update-gnahore-tickets-created-by.mjs --obcs OBCS-11493,OBCS-11491,OBCS-11483
 *   ou
 *   node scripts/update-gnahore-tickets-created-by.mjs --file liste-obcs-gnahore.txt
 */
...
// ⚠️ À COMPLÉTER: ID du profil de GNAHORE AMOS (agent support)
// On le cherchera automatiquement si non fourni
const GNAHORE_PROFILE_ID = process.env.GNAHORE_PROFILE_ID || null;
...
function getOBCSKeys() {
  ...
  console.error('');
  console.error('Usage:');
  console.error('  node scripts/update-gnahore-tickets-created-by.mjs --obcs OBCS-11493,OBCS-11491,OBCS-11483');
  console.error('  ou');
  console.error('  node scripts/update-gnahore-tickets-created-by.mjs --file liste-obcs-gnahore.txt');
  process.exit(1);
}

/**
 * Trouve le profile_id de GNAHORE AMOS dans Supabase
 */
async function findGnahoreProfileId() {
  console.log('🔍 Recherche du profil de GNAHORE AMOS...');
  ...
  throw new Error('Profil de GNAHORE AMOS introuvable. Veuillez fournir le profile_id via la variable d\'environnement GNAHORE_PROFILE_ID ou modifier le script.');
}
...
async function updateTicketCreatedBy(jiraIssueKey, profileId, dryRun = false) {
  ...
  if (ticket.created_by === profileId) {
    console.log(`✅ ${jiraIssueKey} - Déjà à jour (created_by = GNAHORE AMOS)`);
  ...
  const newUserName = newUser?.full_name || 'GNAHORE AMOS';
...
async function main() {
  ...
  console.log('🔍 Mise à jour des tickets created_by pour GNAHORE AMOS\n');
  
  if (dryRun) {
    console.log('⚠️  MODE DRY-RUN activé - Aucune modification ne sera effectuée\n');
  }
  
  // 0. Trouver le profile_id de GNAHORE AMOS
  let gnahoreProfileId = GNAHORE_PROFILE_ID;
  
  if (!gnahoreProfileId) {
    gnahoreProfileId = await findGnahoreProfileId();
  } else {
    // Vérifier que le profil existe
    const { data: profile, error: profileError } = await supabase
      .from('profiles')
      .select('id, full_name, email, role')
      .eq('id', gnahoreProfileId)
      .single();
    
    if (profileError || !profile) {
      console.error(`❌ Profil avec ID ${gnahoreProfileId} introuvable:`, profileError?.message);
      process.exit(1);
    }
  ...
  for (const { obcsKey, odKey } of odKeys) {
    const result = await updateTicketCreatedBy(odKey, gnahoreProfileId, dryRun);
