...
/**
 * Script d'import des contacts clients JOEL K PROPERTIES
 * 
 * Usage: node scripts/import-contacts-joel-k-properties.js
 */
...
const contactsData = [
  { "Nom Complet": "Raïssa CAMARA", "Email": "", "Rôle": "" },
  { "Nom Complet": "Joel A, CAMARA Raïssa", "Email": "", "Rôle": "" },
  { "Nom Complet": "Estelle BOA", "Email": "office@joelka.com", "Rôle": "Comptable" },
  { "Nom Complet": "Rachel AKA", "Email": "", "Rôle": "" },
  { "Nom Complet": "Joel A", "Email": "", "Rôle": "" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise JOEL K PROPERTIES...\n`);

  const { data: companies, error: companyErr } = await supabase
    .from('companies')
    .select('id, name, jira_company_id')
    .ilike('name', '%joel%k%properties%');

  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise JOEL K PROPERTIES non trouvée`);
    console.error(`   Erreur: ${companyErr?.message || 'Entreprise introuvable'}`);
    process.exit(1);
  }

  const joelKPropertiesCompany = companies[0];
  console.log(`✅ Entreprise JOEL K PROPERTIES trouvée (ID: ${joelKPropertiesCompany.id}, Nom: ${joelKPropertiesCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', joelKPropertiesCompany.id)
          .single();
        existing = data;
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          full_name: fullName,
          role: 'client',
          company_id: joelKPropertiesCompany.id,
          job_title: jobTitle,
          jira_user_id: null,
          is_active: true
        })
