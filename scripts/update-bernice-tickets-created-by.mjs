...
/**
 * Script pour mettre à jour created_by des tickets créés par N'GBRA MOYE BERNICE DORIS
 * 
 * Processus:
 ...
 * 3. Met à jour tickets.created_by avec le profil de N'GBRA MOYE BERNICE DORIS
 * 
 * Usage:
 *   node scripts/update-bernice-tickets-created-by.mjs --obcs OBCS-11493,OBCS-11491,OBCS-11483
 *   ou
 *   node scripts/update-bernice-tickets-created-by.mjs --file liste-obcs-bernice.txt
 */
...
// ⚠️ À COMPLÉTER: ID du profil de N'GBRA MOYE BERNICE DORIS (agent support)
// On le cherchera automatiquement si non fourni
const BERNICE_PROFILE_ID = process.env.BERNICE_PROFILE_ID || null;
...
function getOBCSKeys() {
  ...
  console.error('');
  console.error('Usage:');
  console.error('  node scripts/update-bernice-tickets-created-by.mjs --obcs OBCS-11493,OBCS-11491,OBCS-11483');
  console.error('  ou');
  console.error('  node scripts/update-bernice-tickets-created-by.mjs --file liste-obcs-bernice.txt');
  process.exit(1);
}

/**
 * Trouve le profile_id de N'GBRA MOYE BERNICE DORIS dans Supabase
 */
async function findBerniceProfileId() {
  console.log('🔍 Recherche du profil de N\'GBRA MOYE BERNICE DORIS...');
  
  // Rechercher par nom (variations possibles)
  const searchNames = [
    "N'GBRA MOYE BERNICE DORIS",
    "NGBRA MOYE BERNICE DORIS",
    "BERNICE DORIS",
    "MOYE BERNICE",
    "N'GBRA MOYE",
    "NGBRA MOYE"
  ...
  throw new Error('Profil de N\'GBRA MOYE BERNICE DORIS introuvable. Veuillez fournir le profile_id via la variable d\'environnement BERNICE_PROFILE_ID ou modifier le script.');
}
...
async function updateTicketCreatedBy(jiraIssueKey, profileId, dryRun = false) {
  ...
  if (ticket.created_by === profileId) {
    console.log(`✅ ${jiraIssueKey} - Déjà à jour (created_by = N'GBRA MOYE BERNICE DORIS)`);
  ...
  const newUserName = newUser?.full_name || 'N\'GBRA MOYE BERNICE DORIS';
...
async function main() {
  ...
  console.log('🔍 Mise à jour des tickets created_by pour N\'GBRA MOYE BERNICE DORIS\n');
  
  if (dryRun) {
    console.log('⚠️  MODE DRY-RUN activé - Aucune modification ne sera effectuée\n');
  }
  
  // 0. Trouver le profile_id de N'GBRA MOYE BERNICE DORIS
  let berniceProfileId = BERNICE_PROFILE_ID;
  
  if (!berniceProfileId) {
    berniceProfileId = await findBerniceProfileId();
  } else {
    // Vérifier que le profil existe
    const { data: profile, error: profileError } = await supabase
      .from('profiles')
      .select('id, full_name, email, role')
      .eq('id', berniceProfileId)
      .single();
    
    if (profileError || !profile) {
      console.error(`❌ Profil avec ID ${berniceProfileId} introuvable:`, profileError?.message);
      process.exit(1);
    }
  ...
  for (const { obcsKey, odKey } of odKeys) {
    const result = await updateTicketCreatedBy(odKey, berniceProfileId, dryRun);
