...
/**
 * Script d'import des contacts clients AFRIC URBA
 * 
 * Usage: node scripts/import-contacts-afric-urba.js
 * 
...
// Données des contacts AFRIC URBA
const contactsData = [
  { "Nom Complet": "Mme BRISSI", "Email": "", "Rôle": "" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise AFRIC URBA...\n`);

  // Récupérer l'ID de l'entreprise AFRIC URBA
  const { data: companies, error: companyErr } = await supabase
    .from('companies')
    .select('id, name, jira_company_id')
    .ilike('name', '%afric%urba%');

  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise AFRIC URBA non trouvée`);
  ...
  const africUrbaCompany = companies[0];
  console.log(`✅ Entreprise AFRIC URBA trouvée (ID: ${africUrbaCompany.id}, Nom: ${africUrbaCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', africUrbaCompany.id)
          .single();
        existing = data;
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          full_name: fullName,
          role: 'client',
          company_id: africUrbaCompany.id,
