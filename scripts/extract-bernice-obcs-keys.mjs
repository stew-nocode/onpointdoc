...
/**
 * Script pour extraire les clés OBCS des tickets créés par N'GBRA MOYE BERNICE DORIS
 * depuis le Google Sheet filtré
 * 
 * Usage:
 *   node scripts/extract-bernice-obcs-keys.mjs [--output fichier.txt]
 */
...
// URL du Google Sheet filtré sur N'GBRA MOYE BERNICE DORIS
...
/**
 * Parse le CSV et extrait les clés OBCS des tickets créés par N'GBRA MOYE BERNICE DORIS
 */
function extractOBCSKeys(csvText) {
  ...
  // Filtrer sur N'GBRA MOYE BERNICE DORIS et extraire les clés OBCS
  const obcsKeys = [];
  const berniceVariants = [
    "n'gbra", 
    "ngbra", 
    "moye", 
    "bernice", 
  ...
  for (const record of records) {
    ...
    // Vérifier si le créateur correspond à N'GBRA MOYE BERNICE DORIS (insensible à la casse)
    const creatorLower = creator.toLowerCase();
    const isBernice = berniceVariants.some(variant => creatorLower.includes(variant.toLowerCase()));
    
    if (isBernice && obcsKey && (obcsKey.startsWith('OBCS-') || obcsKey.startsWith('OBBCS-') || obcsKey.startsWith('OBCSS-'))) {
      obcsKeys.push(obcsKey);
    }
  }
...
async function main() {
  ...
  console.log('🔍 EXTRACTION DES CLÉS OBCS - TICKETS CRÉÉS PAR N\'GBRA MOYE BERNICE DORIS\n');
  ...
  try {
    ...
    const outputPath = outputFile 
      ...
      : path.resolve(__dirname, '../liste-obcs-bernice.txt');
    ...
    console.log(`   node scripts/update-bernice-tickets-created-by.mjs --file ${path.basename(outputPath)}`);
    console.log('');
