...
/**
 * Script pour mettre à jour created_by des tickets créés par Edwige KOUASSI
 * 
 * Processus:
 ...
 * 3. Met à jour tickets.created_by avec le profil d'Edwige KOUASSI
 * 
 * Usage:
...
// ✅ ID du profil d'Edwige KOUASSI (agent support)
const EDWIGE_PROFILE_ID = 'ff6b3d35-c635-4258-a253-db3fac202302';
...
function getOBCSKeys() {
  ...
  console.error('');
  console.error('Usage:');
  console.error('  node scripts/update-edwige-tickets-created-by.mjs --obcs OBCS-11493,OBCS-11491,OBCS-11483');
  console.error('  ou');
  console.error('  node scripts/update-edwige-tickets-created-by.mjs --file liste-obcs.txt');
  process.exit(1);
}
...
async function updateTicketCreatedBy(jiraIssueKey, dryRun = false) {
  ...
  if (ticket.created_by === EDWIGE_PROFILE_ID) {
    console.log(`✅ ${jiraIssueKey} - Déjà à jour (created_by = Edwige KOUASSI)`);
  ...
  if (dryRun) {
    ...
    console.log(`   Nouveau: Edwige KOUASSI`);
  ...
  const { error: updateError } = await supabase
    .from('tickets')
    .update({ created_by: EDWIGE_PROFILE_ID })
    .eq('id', ticket.id);
  ...
  console.log(`✅ ${jiraIssueKey} - Mis à jour (${currentUserName} → Edwige KOUASSI)`);
...
async function main() {
  ...
  console.log('🔍 Mise à jour des tickets created_by pour Edwige KOUASSI\n');
  ...
  console.log(`✅ ${obcsKeys.length} clés OBCS à traiter:`, obcsKeys.join(', '));
  console.log('');
  ...
  if (notFound.length > 0) {
    console.warn(`⚠️  ${notFound.length} clés OBCS sans correspondance OD:`, notFound.join(', '));
    console.log('');
  }
  ...
  odKeys.forEach(({ obcsKey, odKey }) => {
    console.log(`   ${obcsKey} → ${odKey}`);
  });
  console.log('');
  
  // 4. Vérifier le profil d'Edwige
  console.log('🔍 Vérification du profil d\'Edwige KOUASSI...');
  const { data: edwigeProfile, error: profileError } = await supabase
    .from('profiles')
    .select('id, full_name, email, role')
    .eq('id', EDWIGE_PROFILE_ID)
    .single();
  
  if (profileError || !edwigeProfile) {
    console.error(`❌ Profil d'Edwige KOUASSI introuvable:`, profileError?.message);
    process.exit(1);
  }
  
  console.log(`✅ Profil trouvé: ${edwigeProfile.full_name} (${edwigeProfile.email}, ${edwigeProfile.role})`);
  console.log('');
