...
/**
 * Script d'import des contacts clients SIE-TRAVAUX
 * 
 * Usage: node scripts/import-contacts-sie-travaux.js
 */
...
const contactsData = [
  { "Nom Complet": "KOUAME BRICE", "Email": "kouameb@sie-travaux.ci", "Rôle": "Comptable" },
  { "Nom Complet": "OUSSOU Martine", "Email": "oussoumartine@sie-travaux.ci", "Rôle": "Assistante de Direction" },
  { "Nom Complet": "MINGA Somgbo", "Email": "mingas@sie-travaux.ci", "Rôle": "Directeur Technique" },
  { "Nom Complet": "Kouamé Stéphane", "Email": "kouamestephane@sie-travaux.ci", "Rôle": "Contrôleur de Gestion" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise SIE-TRAVAUX...\n`);

  const { data: companies, error: companyErr } = await supabase
    .from('companies')
    .select('id, name, jira_company_id')
    .ilike('name', '%sie%travaux%');

  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise SIE-TRAVAUX non trouvée`);
    console.error(`   Erreur: ${companyErr?.message || 'Entreprise introuvable'}`);
    process.exit(1);
  }

  const sieTravauxCompany = companies[0];
  console.log(`✅ Entreprise SIE-TRAVAUX trouvée (ID: ${sieTravauxCompany.id}, Nom: ${sieTravauxCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', sieTravauxCompany.id)
          .single();
        existing = data;
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          full_name: fullName,
          role: 'client',
          company_id: sieTravauxCompany.id,
          job_title: jobTitle,
          jira_user_id: null,
          is_active: true
        })
