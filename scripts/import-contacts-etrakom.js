...
/**
 * Script d'import des contacts clients ETRAKOM-CI
 * 
 * Usage: node scripts/import-contacts-etrakom.js
 * 
...
// Données des contacts ETRAKOM-CI
const contactsData = [
  { "Nom Complet": "OUATTARA", "Email": "", "Rôle": "" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise ETRAKOM-CI...\n`);

  // Récupérer l'ID de l'entreprise ETRAKOM-CI
  ...
  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise ETRAKOM-CI non trouvée`);
  ...
  const etrakomCompany = companies[0];
  console.log(`✅ Entreprise ETRAKOM-CI trouvée (ID: ${etrakomCompany.id}, Nom: ${etrakomCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', etrakomCompany.id)
          .single();
        existing = data;
      }
