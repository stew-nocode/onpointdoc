...
/**
 * Script d'import des contacts clients ETS MAB
 * 
 * Usage: node scripts/import-contacts-ets-mab.js
 */
...
const contactsData = [
  { "Nom Complet": "Rodrigue KOUAKOU", "Email": "rodriguekouakou@etsmab.com", "Rôle": "Directeur Administratif et Financier" },
  { "Nom Complet": "Diaby Nour", "Email": "nourdiaby@etsmab.com", "Rôle": "Responsable achat et logistique" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise ETS MAB...\n`);
  ...
  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise ETS MAB non trouvée`);
    console.error(`   Erreur: ${companyErr?.message || 'Entreprise introuvable'}`);
    process.exit(1);
  }

  const etsMabCompany = companies[0];
  console.log(`✅ Entreprise ETS MAB trouvée (ID: ${etsMabCompany.id}, Nom: ${etsMabCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', etsMabCompany.id)
          .single();
        existing = data;
      }
