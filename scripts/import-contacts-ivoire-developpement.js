...
/**
 * Script d'import des contacts clients IVOIRE DEVELOPPEMENT
 * 
 * Usage: node scripts/import-contacts-ivoire-developpement.js
 */
...
const contactsData = [
  { "Nom Complet": "GAHI Lezou Jean", "Email": "", "Rôle": "" },
  { "Nom Complet": "COULIBALY EVE", "Email": "eve.kouadio.ek@gmail.com", "Rôle": "Responsable des Ressources Humaines" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise IVOIRE DEVELOPPEMENT...\n`);

  const { data: companies, error: companyErr } = await supabase
    .from('companies')
    .select('id, name, jira_company_id')
    .ilike('name', '%ivoire%developpement%');

  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise IVOIRE DEVELOPPEMENT non trouvée`);
    console.error(`   Erreur: ${companyErr?.message || 'Entreprise introuvable'}`);
    process.exit(1);
  }

  const ivoireDeveloppementCompany = companies[0];
  console.log(`✅ Entreprise IVOIRE DEVELOPPEMENT trouvée (ID: ${ivoireDeveloppementCompany.id}, Nom: ${ivoireDeveloppementCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', ivoireDeveloppementCompany.id)
          .single();
        existing = data;
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          full_name: fullName,
          role: 'client',
          company_id: ivoireDeveloppementCompany.id,
          job_title: jobTitle,
          jira_user_id: null,
          is_active: true
        })
