...
/**
 * Script d'import des contacts clients CSCTICAO
 * 
 * Usage: node scripts/import-contacts-cscticao.js
 * 
...
// Données des contacts CSCTICAO
const contactsData = [
  { "Nom Complet": "SERGE ARIKO", "Email": "", "Rôle": "" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise CSCTICAO...\n`);

  // Récupérer l'ID de l'entreprise CSCTICAO
  ...
  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise CSCTICAO non trouvée`);
  ...
  const cscticaoCompany = companies[0];
  console.log(`✅ Entreprise CSCTICAO trouvée (ID: ${cscticaoCompany.id}, Nom: ${cscticaoCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', cscticaoCompany.id)
          .single();
        existing = data;
      }
