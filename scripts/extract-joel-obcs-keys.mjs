...
/**
 * Script pour extraire les clés OBCS des tickets créés par JOEL SIE
 * depuis le Google Sheet filtré
 * 
 * Usage:
 *   node scripts/extract-joel-obcs-keys.mjs [--output fichier.txt]
 */
...
// URL du Google Sheet filtré sur JOEL SIE
const GOOGLE_SHEETS_ID = '1M3FraNFTqqanqEjaVA0r957KfNUuNARU6mZBERGpnq8';
const GID = '701656857'; // Mettre à jour si différent pour le filtre JOEL SIE
...
/**
 * Parse le CSV et extrait les clés OBCS des tickets créés par JOEL SIE
 */
function extractOBCSKeys(csvText) {
  ...
  // Filtrer sur JOEL SIE et extraire les clés OBCS
  const obcsKeys = [];
  const joelVariants = ['joel', 'sie', 'joel sie', 'joël sie', 'joel sie', 'joël'];
  ...
  for (const record of records) {
    ...
    // Vérifier si le créateur correspond à JOEL SIE (insensible à la casse)
    const creatorLower = creator.toLowerCase();
    const isJoel = joelVariants.some(variant => creatorLower.includes(variant));
    
    if (isJoel && obcsKey && (obcsKey.startsWith('OBCS-') || obcsKey.startsWith('OBBCS-') || obcsKey.startsWith('OBCSS-'))) {
      obcsKeys.push(obcsKey);
    }
  }
...
async function main() {
  ...
  console.log('🔍 EXTRACTION DES CLÉS OBCS - TICKETS CRÉÉS PAR JOEL SIE\n');
  ...
  try {
    ...
    const outputPath = outputFile 
      ...
      : path.resolve(__dirname, '../liste-obcs-joel.txt');
    ...
    console.log(`   node scripts/update-joel-tickets-created-by.mjs --file ${path.basename(outputPath)}`);
    console.log('');
