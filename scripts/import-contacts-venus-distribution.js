...
/**
 * Script d'import des contacts clients VENUS DISTRIBUTION
 * 
 * Usage: node scripts/import-contacts-venus-distribution.js
 */
...
const contactsData = [
  { "Nom Complet": "Koné", "Email": "", "Rôle": "" },
  { "Nom Complet": "KONATE TAYOU", "Email": "venus.sarl.comptabilite@gmail.com", "Rôle": "Comptable" },
  { "Nom Complet": "DAF Venus", "Email": "", "Rôle": "" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise VENUS DISTRIBUTION...\n`);

  const { data: companies, error: companyErr } = await supabase
    .from('companies')
    .select('id, name, jira_company_id')
    .ilike('name', '%venus%distribution%');

  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise VENUS DISTRIBUTION non trouvée`);
    console.error(`   Erreur: ${companyErr?.message || 'Entreprise introuvable'}`);
    process.exit(1);
  }

  const venusDistributionCompany = companies[0];
  console.log(`✅ Entreprise VENUS DISTRIBUTION trouvée (ID: ${venusDistributionCompany.id}, Nom: ${venusDistributionCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', venusDistributionCompany.id)
          .single();
        existing = data;
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          full_name: fullName,
          role: 'client',
          company_id: venusDistributionCompany.id,
          job_title: jobTitle,
          jira_user_id: null,
          is_active: true
        })
