...
/**
 * Script d'import des contacts clients ROCFED
 * 
 * Usage: node scripts/import-contacts-rocfed.js
 */
...
const contactsData = [
  { "Nom Complet": "DOUMBIA", "Email": "", "Rôle": "" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise ROCFED...\n`);
  ...
  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise ROCFED non trouvée`);
    console.error(`   Erreur: ${companyErr?.message || 'Entreprise introuvable'}`);
    process.exit(1);
  }

  const rocfedCompany = companies[0];
  console.log(`✅ Entreprise ROCFED trouvée (ID: ${rocfedCompany.id}, Nom: ${rocfedCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', rocfedCompany.id)
          .single();
        existing = data;
      }
