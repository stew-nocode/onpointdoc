...
/**
 * Script d'import des contacts clients ECORIGINE
 * 
 * Usage: node scripts/import-contacts-ecorigine.js
 * 
...
// Données des contacts ECORIGINE
const contactsData = [
  { "Nom Complet": "MICHEL TETE", "Email": "michel.tete@ecorigine.ci", "Rôle": "Contrôleur de Gestion" },
  { "Nom Complet": "Souleymane CISSE", "Email": "souleymane.cisse@ecorigine.ci", "Rôle": "Responsable Administrative" },
  { "Nom Complet": "Serge AMOUZOU", "Email": "serge.amouzoun@ecorigine.ci", "Rôle": "Chargé Achat & Durabilité" },
  { "Nom Complet": "Ismaël KONE", "Email": "ismael.kone@ecorigine.ci", "Rôle": "Administrateur général" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise ECORIGINE...\n`);

  // Récupérer l'ID de l'entreprise ECORIGINE
  const { data: companies, error: companyErr } = await supabase
    .from('companies')
    .select('id, name, jira_company_id')
    .ilike('name', '%ecorigine%');

  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise ECORIGINE non trouvée`);
  ...
  const ecorigineCompany = companies[0];
  console.log(`✅ Entreprise ECORIGINE trouvée (ID: ${ecorigineCompany.id}, Nom: ${ecorigineCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', ecorigineCompany.id)
          .single();
        existing = data;
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          full_name: fullName,
          role: 'client',
          company_id: ecorigineCompany.id,
