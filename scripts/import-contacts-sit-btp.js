...
/**
 * Script d'import des contacts clients SIT BTP
 * 
 * Usage: node scripts/import-contacts-sit-btp.js
 */
...
const contactsData = [
  { "Nom Complet": "KONE Mariam", "Email": "m.kone@sit-btp.com", "Rôle": "Contrôleur de Gestion" },
  { "Nom Complet": "KOUAME KONAN GUY ROGER", "Email": "gr.kouame@sit-btp.com", "Rôle": "Directeur General" },
  { "Nom Complet": "ETEKOU ETEKOU JEAN CHRISTOPHE", "Email": "", "Rôle": "Chargé d'Achat et Logistique" },
  { "Nom Complet": "M. SIABA", "Email": "", "Rôle": "" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise SIT BTP...\n`);
  ...
  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise SIT BTP non trouvée`);
    console.error(`   Erreur: ${companyErr?.message || 'Entreprise introuvable'}`);
    process.exit(1);
  }

  const sitBtpCompany = companies[0];
  console.log(`✅ Entreprise SIT BTP trouvée (ID: ${sitBtpCompany.id}, Nom: ${sitBtpCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', sitBtpCompany.id)
          .single();
        existing = data;
      }
