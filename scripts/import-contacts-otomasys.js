...
/**
 * Script d'import des contacts clients OTOMASYS
 * 
 * Usage: node scripts/import-contacts-otomasys.js
 */
...
const contactsData = [
  { "Nom Complet": "Aude KOFFI", "Email": "aude.iyen@otomasys.com", "Rôle": "Comptable" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise OTOMASYS...\n`);
  ...
  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise OTOMASYS non trouvée`);
    console.error(`   Erreur: ${companyErr?.message || 'Entreprise introuvable'}`);
    process.exit(1);
  }

  const otomasysCompany = companies[0];
  console.log(`✅ Entreprise OTOMASYS trouvée (ID: ${otomasysCompany.id}, Nom: ${otomasysCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', otomasysCompany.id)
          .single();
        existing = data;
      }
