 ...
 * 
 * Exécution:
 *   node --loader ts-node/esm scripts/seed-marketing-user.ts
 *   ou compiler avec tsc puis exécuter avec node
 */
...
// ID du département Marketing (récupéré depuis la base)
...
/**
 * Crée ou récupère un utilisateur Auth
 */
async function ensureAuthUser(email: string, password: string) {
  try {
    ...
    throw new Error('Impossible de créer ou récupérer l\'utilisateur Auth');
  } catch (error: any) {
 ...
 */
async function ensureProfile(authUid: string) {
...
/**
 * Crée 5 tâches pour l'utilisateur
 */
async function createTasks(profileId: string) {
  ...
  const tasks = [
    {
      ...
      start_date: new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000).toISOString(), // Demain
      estimated_duration_hours: 3.5,
      actual_duration_hours: null, // Pas encore complétée
    ...
    },
    {
      ...
      start_date: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000).toISOString(), // Il y a 2 jours
      estimated_duration_hours: 6,
      actual_duration_hours: 7.5, // Complétée avec durée réelle
      assigned_to: profileId,
      status: 'Termine',
      is_planned: true,
      created_by: profileId
    },
    {
      ...
      start_date: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000).toISOString(), // Hier
      estimated_duration_hours: 2,
      actual_duration_hours: 2.5, // Complétée avec durée réelle
      assigned_to: profileId,
      status: 'Termine',
      is_planned: true,
      created_by: profileId
...
/**
 * Crée 5 activités pour l'utilisateur
 */
async function createActivities(profileId: string) {
  const today = new Date();
  
  // Dates pour les activités planifiées
  ...
  const activities = [
    {
      ...
      estimated_duration_hours: 2, // Utilise estimated_duration_hours
      actual_duration_hours: null,
      status: 'Planifie',
      created_by: profileId
    },
    ...
    },
    {
      ...
      estimated_duration_hours: 2.5,
      actual_duration_hours: 3, // Complétée avec durée réelle (plus longue que prévu)
      status: 'Termine',
      created_by: profileId
    },
    {
      ...
      estimated_duration_hours: 2,
      actual_duration_hours: 1.75, // Complétée avec durée réelle (plus rapide que prévu)
      status: 'Termine',
      created_by: profileId
    }
  ];
  ...
  const participants = data.map((activity: { id: string }) => ({
