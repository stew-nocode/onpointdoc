...
/**
 * Script d'import des contacts clients FALCON
 * 
 * Usage: node scripts/import-contacts-falcon.js
 */
...
const contactsData = [
  { "Nom Complet": "ERIC KOUADIO", "Email": "e.kouadio@falconsystems.ci", "Rôle": "Comptable" },
  { "Nom Complet": "MONSIEUR KOUASSI", "Email": "", "Rôle": "" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise FALCON...\n`);
  ...
  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise FALCON non trouvée`);
    console.error(`   Erreur: ${companyErr?.message || 'Entreprise introuvable'}`);
    process.exit(1);
  }

  const falconCompany = companies[0];
  console.log(`✅ Entreprise FALCON trouvée (ID: ${falconCompany.id}, Nom: ${falconCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', falconCompany.id)
          .single();
        existing = data;
      }
