...
/**
 * Script pour mettre à jour created_by des tickets créés par Vivien DAKPOGAN
 * 
 * Processus:
 ...
 * 3. Met à jour tickets.created_by avec le profil de Vivien DAKPOGAN
 * 
 * Usage:
 *   node scripts/update-vivien-tickets-created-by.mjs --obcs OBCS-11493,OBCS-11491,OBCS-11483
 *   ou
 *   node scripts/update-vivien-tickets-created-by.mjs --file liste-obcs-vivien.txt
 */
...
// ⚠️ À COMPLÉTER: ID du profil de Vivien DAKPOGAN (agent support)
// On le cherchera automatiquement si non fourni
const VIVIEN_PROFILE_ID = process.env.VIVIEN_PROFILE_ID || null;
...
function getOBCSKeys() {
  ...
  console.error('');
  console.error('Usage:');
  console.error('  node scripts/update-vivien-tickets-created-by.mjs --obcs OBCS-11493,OBCS-11491,OBCS-11483');
  console.error('  ou');
  console.error('  node scripts/update-vivien-tickets-created-by.mjs --file liste-obcs-vivien.txt');
  process.exit(1);
}

/**
 * Trouve le profile_id de Vivien DAKPOGAN dans Supabase
 */
async function findVivienProfileId() {
  console.log('🔍 Recherche du profil de Vivien DAKPOGAN...');
  ...
  throw new Error('Profil de Vivien DAKPOGAN introuvable. Veuillez fournir le profile_id via la variable d\'environnement VIVIEN_PROFILE_ID ou modifier le script.');
}
...
async function updateTicketCreatedBy(jiraIssueKey, profileId, dryRun = false) {
  ...
  if (ticket.created_by === profileId) {
    console.log(`✅ ${jiraIssueKey} - Déjà à jour (created_by = Vivien DAKPOGAN)`);
  ...
  const newUserName = newUser?.full_name || 'Vivien DAKPOGAN';
...
async function main() {
  ...
  console.log('🔍 Mise à jour des tickets created_by pour Vivien DAKPOGAN\n');
  
  if (dryRun) {
    console.log('⚠️  MODE DRY-RUN activé - Aucune modification ne sera effectuée\n');
  }
  
  // 0. Trouver le profile_id de Vivien DAKPOGAN
  let vivienProfileId = VIVIEN_PROFILE_ID;
  
  if (!vivienProfileId) {
    vivienProfileId = await findVivienProfileId();
  } else {
    // Vérifier que le profil existe
    const { data: profile, error: profileError } = await supabase
      .from('profiles')
      .select('id, full_name, email, role')
      .eq('id', vivienProfileId)
      .single();
    
    if (profileError || !profile) {
      console.error(`❌ Profil avec ID ${vivienProfileId} introuvable:`, profileError?.message);
      process.exit(1);
    }
  ...
  for (const { obcsKey, odKey } of odKeys) {
    const result = await updateTicketCreatedBy(odKey, vivienProfileId, dryRun);
