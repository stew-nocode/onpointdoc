...
/**
 * Script d'import des sous-modules Finance depuis JIRA
 * 
 * Usage: node scripts/import-submodules-finance.js
 * 
...
// Données des sous-modules Finance depuis JIRA
const submodulesData = [
  { "Nom Sous-Module": "Comptabilité Générale", "ID Jira Sous-Module": 10839 },
  { "Nom Sous-Module": "Comptabilité analytique", "ID Jira Sous-Module": 10837 },
  { "Nom Sous-Module": "Impôts et taxes", "ID Jira Sous-Module": 10841 },
  { "Nom Sous-Module": "Trésorerie", "ID Jira Sous-Module": 10844 },
  { "Nom Sous-Module": "Budget", "ID Jira Sous-Module": 10838 },
  { "Nom Sous-Module": "Etats Financiers", "ID Jira Sous-Module": 10836 }
];

async function importSubmodules() {
  console.log(`\n🔍 Recherche du module Finance...\n`);

  // Récupérer l'ID du module Finance (JIRA ID: 10029)
  const { data: financeModule, error: moduleErr } = await supabase
    .from('modules')
    .select('id, name, id_module_jira')
    .eq('id_module_jira', 10029)
    .single();

  if (moduleErr || !financeModule) {
    console.error(`❌ Module Finance non trouvé (JIRA ID: 10029)`);
    console.error(`   Erreur: ${moduleErr?.message || 'Module introuvable'}`);
    process.exit(1);
  }

  console.log(`✅ Module Finance trouvé (ID: ${financeModule.id}, Nom: ${financeModule.name})\n`);
