...
/**
 * Script d'import des sous-modules RH depuis JIRA
 * 
 * Usage: node scripts/import-submodules-rh.js
 * 
...
// Données des sous-modules RH depuis JIRA
const submodulesData = [
  { "Nom Sous-Module": "Gestion employé", "ID Jira Sous-Module": 10853 },
  { "Nom Sous-Module": "Salaire", "ID Jira Sous-Module": 10855 },
  { "Nom Sous-Module": "Documents", "ID Jira Sous-Module": 10851 },
  { "Nom Sous-Module": "Paramétrage", "ID Jira Sous-Module": 10854 }
];

async function importSubmodules() {
  console.log(`\n🔍 Recherche du module RH...\n`);

  // Récupérer l'ID du module RH (JIRA ID: 10031)
  const { data: rhModule, error: moduleErr } = await supabase
    .from('modules')
    .select('id, name, id_module_jira')
    .eq('id_module_jira', 10031)
    .single();

  if (moduleErr || !rhModule) {
    console.error(`❌ Module RH non trouvé (JIRA ID: 10031)`);
    console.error(`   Erreur: ${moduleErr?.message || 'Module introuvable'}`);
    process.exit(1);
  }

  console.log(`✅ Module RH trouvé (ID: ${rhModule.id}, Nom: ${rhModule.name})\n`);
