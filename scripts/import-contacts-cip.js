...
/**
 * Script d'import des contacts clients CIP
 * 
 * Usage: node scripts/import-contacts-cip.js
 * 
...
// Données des contacts CIP
const contactsData = [
  { "Nom Complet": "Jonathan N'DOUBA", "Email": "", "Rôle": "" },
  { "Nom Complet": "Marina NATCHIA", "Email": "", "Rôle": "" },
  { "Nom Complet": "OURAGA Edwige", "Email": "", "Rôle": "" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise CIP...\n`);

  // Récupérer l'ID de l'entreprise CIP
  ...
  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise CIP non trouvée`);
  ...
  const cipCompany = companies[0];
  console.log(`✅ Entreprise CIP trouvée (ID: ${cipCompany.id}, Nom: ${cipCompany.name})\n`);
