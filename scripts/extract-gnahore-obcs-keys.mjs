...
/**
 * Script pour extraire les clés OBCS des tickets créés par GNAHORE AMOS
 * depuis le Google Sheet filtré
 * 
 * Usage:
 *   node scripts/extract-gnahore-obcs-keys.mjs [--output fichier.txt]
 */
...
// URL du Google Sheet filtré sur GNAHORE AMOS
const GOOGLE_SHEETS_ID = '1M3FraNFTqqanqEjaVA0r957KfNUuNARU6mZBERGpnq8';
const GID = '701656857'; // Mettre à jour si différent pour le filtre GNAHORE AMOS
...
/**
 * Parse le CSV et extrait les clés OBCS des tickets créés par GNAHORE AMOS
 */
function extractOBCSKeys(csvText) {
  ...
  // Filtrer sur GNAHORE AMOS et extraire les clés OBCS
  const obcsKeys = [];
  const gnahoreVariants = ['gnahore', 'amos', 'gnahore amos', 'amos gnahore'];
  ...
  for (const record of records) {
    ...
    // Vérifier si le créateur correspond à GNAHORE AMOS (insensible à la casse)
    const creatorLower = creator.toLowerCase();
    const isGnahore = gnahoreVariants.some(variant => creatorLower.includes(variant));
    
    if (isGnahore && obcsKey && (obcsKey.startsWith('OBCS-') || obcsKey.startsWith('OBBCS-') || obcsKey.startsWith('OBCSS-'))) {
      obcsKeys.push(obcsKey);
    }
  }
...
async function main() {
  ...
  console.log('🔍 EXTRACTION DES CLÉS OBCS - TICKETS CRÉÉS PAR GNAHORE AMOS\n');
  ...
  try {
    ...
    const outputPath = outputFile 
      ...
      : path.resolve(__dirname, '../liste-obcs-gnahore.txt');
    ...
    console.log(`   node scripts/update-gnahore-tickets-created-by.mjs --file ${path.basename(outputPath)}`);
    console.log('');
