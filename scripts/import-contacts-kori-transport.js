...
/**
 * Script d'import des contacts clients KORI TRANSPORT
 * 
 * Usage: node scripts/import-contacts-kori-transport.js
 */
...
const contactsData = [
  { "Nom Complet": "Brahima COULIBALY", "Email": "brahima.coulibaly@koritransport.com", "Rôle": "Analyste Data" },
  { "Nom Complet": "Mme Traoré", "Email": "", "Rôle": "" },
  { "Nom Complet": "Bénédicte ZONTODJI", "Email": "stock@koritransport.com", "Rôle": "Magasinier" },
  { "Nom Complet": "M. ECARE", "Email": "", "Rôle": "" },
  { "Nom Complet": "CHEICK SANGARE", "Email": "", "Rôle": "" },
  { "Nom Complet": "Delphin Zoungrana", "Email": "delphin.zoungrana@koritransport.com", "Rôle": "" },
  { "Nom Complet": "M. Traoré (SAEFI)", "Email": "", "Rôle": "" },
  { "Nom Complet": "Cabinet SAEFI", "Email": "", "Rôle": "" },
  { "Nom Complet": "CISSOKO Mamadou", "Email": "", "Rôle": "" },
  { "Nom Complet": "KOUMAN", "Email": "", "Rôle": "" },
  { "Nom Complet": "ANGUIDOU", "Email": "", "Rôle": "" }
];

async function importContacts() {
  console.log(`\n🔍 Recherche de l'entreprise KORI TRANSPORT...\n`);

  const { data: companies, error: companyErr } = await supabase
    .from('companies')
    .select('id, name, jira_company_id')
    .ilike('name', '%kori%transport%');

  if (companyErr || !companies || companies.length === 0) {
    console.error(`❌ Entreprise KORI TRANSPORT non trouvée`);
    console.error(`   Erreur: ${companyErr?.message || 'Entreprise introuvable'}`);
    process.exit(1);
  }

  const koriTransportCompany = companies[0];
  console.log(`✅ Entreprise KORI TRANSPORT trouvée (ID: ${koriTransportCompany.id}, Nom: ${koriTransportCompany.name})\n`);
    ...
    try {
      ...
      if (email) {
        ...
        const { data } = await supabase
          ...
          .eq('company_id', koriTransportCompany.id)
          .single();
        existing = data;
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          full_name: fullName,
          role: 'client',
          company_id: koriTransportCompany.id,
          job_title: jobTitle,
          jira_user_id: null,
          is_active: true
        })
